...
import { createItem, editItem } from './formSlice';
...
export const getItems = createAsyncThunk('items/getItems', async (__, { fulfillWithValue }) => {
  try {
...
export const deleteItems = createAsyncThunk(
  ...
  async (id, { fulfillWithValue, rejectWithValue }) => {
    try {
      ...
      if (res.status === 'error') return rejectWithValue();
      console.log(res)
...
export const { selectAll: selectItems } = itemsAdapter.getSelectors((state) => state.items);
...
const itemsSlice = createSlice({
  ...
  extraReducers: (b) => {
    ...
    b.addCase(editItem.fulfilled, (state, action) => {
      itemsAdapter.upsertOne(state, action.payload);
    });
    b.addCase(deleteItems.fulfilled, (state, action) => {
